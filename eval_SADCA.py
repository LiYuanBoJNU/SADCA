...
from SADCA_Attacker import Attacker, ImageAttacker, TextAttacker
...
def save_img(save_path, norm_img):
    pil_array = (norm_img * 255).to(torch.uint8).cpu().numpy()
    pil_img = Image.fromarray(np.transpose(pil_array, (1, 2, 0)))
...
def retrieval_eval(model, ref_model, t_models, t_ref_models, t_test_transforms, data_loader, tokenizer, t_tokenizers,
    ...
    img_attacker = ImageAttacker(images_normalize, eps=8 / 255, steps=10, step_size=2 / 255, lam=args.lam, input_trans_num=args.input_trans_num,  max_length=max_length)
    txt_attacker = TextAttacker(ref_model, tokenizer, cls=False, max_length=max_length, number_perturbation=1,
                                topk=10, threshold_pred_score=0.3, lam=args.lam)
    attacker = Attacker(model, img_attacker, txt_attacker, tokenizer, args.lam, args.dynamic_num)
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups, image_paths) in enumerate(data_loader):
        ...
        with torch.no_grad():
            txts_input = tokenizer(texts, padding='max_length', truncation=True, max_length=max_length, return_tensors="pt").to(device)
            txts_output = model.inference_text(txts_input)
            txt_supervisions = txts_output['text_feat']
            origin_img_output = model.inference_image(images_normalize(imgs))
            img_supervisions = origin_img_output['image_feat']
            all_txt_supervisions.append(txt_supervisions)
            all_img_supervisions.append(img_supervisions)
    all_txt_supervisions = torch.cat(all_txt_supervisions, dim=0)
    all_img_supervisions = torch.cat(all_img_supervisions, dim=0)

    print(all_txt_supervisions.size())
    print(all_img_supervisions.size())
    # sys.exit()
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups, image_paths) in enumerate(data_loader):
        ...
        for i in range(len(texts_group)):
            ...
            texts_ids += text_ids_groups[i]
            txt2img += [i] * len(text_ids_groups[i])
            txt2img_global += [images_ids[i]] * len(text_ids_groups[i])
        ...
        with torch.no_grad():
            ...
            txts_output = model.inference_text(txts_input)
            txt_supervisions = txts_output['text_feat']
            origin_img_output = model.inference_image(images_normalize(images))
            img_supervisions = origin_img_output['image_feat']


        topk = args.topk_neg
        sim_txt2img = F.normalize(txt_supervisions, dim=1) @ F.normalize(all_img_supervisions, dim=1).T  # 每个文本与所有图片的相似度
        # Block own images
        txt2img_tensor = torch.tensor(txt2img_global)  # [10]
        mask = torch.zeros_like(sim_txt2img, dtype=torch.bool)
        mask[torch.arange(len(txt2img)), txt2img_tensor] = True  # 自己的位置设为True
        # Set self similarity to -inf to prevent being selected
        sim_txt2img[mask] = -float('inf')

        # # Get the top-k image index of the most similar (excluding itself) image for each text
        # neg_img_idx = torch.topk(sim_txt2img, k=topk, dim=1).indices  # shape: [num_texts, topk]
        # # Get the top-k index of the most dissimilar (excluding itself) image for each text
        # neg_img_idx = torch.topk(sim_txt2img, k=topk, dim=1, largest=False).indices  # shape: [num_texts, topk]

        # # The similarity is sorted and the middle topk image indices after sorting are taken
        # sorted_idx = torch.argsort(sim_txt2img, dim=1)  # [num_texts, num_images]
        # num_images = sim_txt2img.size(1)
        # start = (num_images - topk) // 2
        # neg_img_idx = sorted_idx[:, start:start + topk]  # shape: [num_texts, topk]

        # For each text, k images are randomly selected from non-own images
        neg_img_idx = []
        for i in range(sim_txt2img.size(0)):
            available_idx = (~mask[i]).nonzero(as_tuple=True)[0]  # 非自身图像的索引
            rand_idx = available_idx[torch.randperm(len(available_idx))[:topk]]  # 随机选k个
            neg_img_idx.append(rand_idx)
        neg_img_idx = torch.stack(neg_img_idx)  # shape: [num_texts, topk]

        sim_img2txt = F.normalize(img_supervisions, dim=1) @ F.normalize(all_txt_supervisions, dim=1).T  # 每个图片与所有文本的相似度
        # Block own text
        img2txt_mask = torch.zeros_like(sim_img2txt, dtype=torch.bool)  # [2, 10]
        for i, text_ids in enumerate(text_ids_groups):
            img2txt_mask[i, text_ids] = True  # 每张图像自己的文本设为True
        sim_img2txt[img2txt_mask] = -float('inf')

        # Randomly select topk from non-own texts
        neg_txt_idx = []
        for i in range(sim_img2txt.size(0)):
            available_idx = (~img2txt_mask[i]).nonzero(as_tuple=True)[0]
            rand_idx = available_idx[torch.randperm(len(available_idx))[:topk]]
            neg_txt_idx.append(rand_idx)
        neg_txt_idx = torch.stack(neg_txt_idx).view(-1)  # shape: [num_images * topk]


        neg_img_embeddings = all_img_supervisions[neg_img_idx]
        neg_txt_embeddings = all_txt_supervisions[neg_txt_idx]
        # print('neg_img_idx:', neg_img_idx)
        # print('neg_txt_idx:', neg_txt_idx)
        # sys.exit()

        adv_images, adv_texts, execuate_time = attacker.attack(images, texts, txt2img, neg_img_embeddings, neg_txt_embeddings, device=device,
                                                               max_length=max_length, scales=scales)
        # adv_images, adv_texts = images, texts
        et = time.time()
        print('one adv time: ', et-st)
        # print(texts)
        # print(adv_texts)
        per = adv_images - images
        print('eps:', per.max()*255, per.min()*255)
        image_names = [path.split('/')[-1].replace('.jpg', '.png') for path in image_paths]

        if not os.path.exists(args.save_advimg_path):
            os.makedirs(args.save_advimg_path)
        for i in range(len(adv_images)):
            save_path = args.save_advimg_path + image_names[i]
            print(save_path)
            save_img(save_path, adv_images[i])
    ...
    # Save JSON
    with open(args.save_advimg_caption_path, "w", encoding="utf-8") as f:
        json.dump(adv_data, f, indent=4, ensure_ascii=False)

    print("Saved image_caption_data.json")
...
def eval_asr(model, ref_model, tokenizer, t_models, t_ref_models, t_tokenizers, t_test_transforms, data_loader, device,
    ...
    # result_file_path = "./result_improve-ablation-negtive.txt"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_advimg_caption_path', default='./flickr30k_adv/SADCA_CLIP_CNN.json')


    parser.add_argument('--topk_neg', type=int, default=20)
    parser.add_argument('--lam', type=float, default=0.2)
    parser.add_argument('--dynamic_num', type=int, default=5)
    parser.add_argument('--input_trans_num', type=int, default=10)
